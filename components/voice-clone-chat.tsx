...
export function VoiceCloneChat({
  ...
  return (
    <section
      ref={sectionRef}
      id="ai-clone"
      aria-labelledby="ai-clone-title"
      className="flex h-[100svh] min-h-0 snap-start snap-always overflow-hidden bg-transparent"
    >
      <div className="mobile-section-safe mx-auto flex h-full min-h-0 w-full max-w-[1440px] flex-col overflow-hidden px-4 pt-20 pb-3 sm:px-5 sm:py-7 xl:px-8 xl:pt-32 xl:pb-8">
        <div className="mobile-slide-intro mx-auto hidden max-w-[48rem] shrink-0 text-center sm:block">
          <p
            className={`mobile-slide-kicker screen-shift text-sm font-medium text-muted-foreground ${isVisible ? "screen-shift-visible screen-shift-delay-1" : ""}`}
          >
            Talk to Your AI clone
          </p>
          <h2
            id="ai-clone-title"
            className={`screen-shift mt-2 text-[clamp(1.55rem,4vw,3.75rem)] leading-[0.96] font-medium tracking-[-0.04em] whitespace-nowrap ${isVisible ? "screen-shift-visible screen-shift-delay-2" : ""}`}
          >
            AI that sounds like you
          </h2>
          <p
            className={`mobile-slide-description screen-shift mx-auto mt-4 max-w-[38rem] text-base leading-relaxed text-muted-foreground sm:text-lg ${isVisible ? "screen-shift-visible screen-shift-delay-3" : ""}`}
          >
            It keeps your vocabulary, pacing, and intent — then adapts to the
            moment and the person you’re talking to.
          </p>
        </div>

        <div
          className={`clone-chat-panel screen-shift mx-auto flex min-h-0 w-full max-w-[48rem] flex-1 flex-col overflow-hidden rounded-[20px] border border-border/80 bg-white shadow-[0_16px_42px_rgba(29,30,34,0.06)] sm:mt-10 lg:mt-8 lg:h-[38rem] lg:max-h-[calc(100svh-20rem)] lg:max-w-[42rem] lg:flex-none ${isVisible ? "screen-shift-visible screen-shift-delay-4" : ""}`}
        >
          <div className="clone-chat-header flex shrink-0 items-center gap-3 border-b border-border/70 px-5 py-4">
            <span className="flex size-10 items-center justify-center rounded-[15px] bg-primary text-primary-foreground">
              <SparklesIcon
                aria-hidden="true"
                className="size-4"
                strokeWidth={1.8}
          ...
          <div
            ref={scrollerRef}
            aria-live="polite"
            className="clone-chat-messages no-scrollbar flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-5 py-5"
          >
            ...
            {isReplying ? (
              <div className="flex justify-start">
                <span className="rounded-[15px] bg-muted/58 px-4 py-3 text-sm text-muted-foreground">
                  Writing
                  <span className="ml-0.5 inline-flex gap-0.5">
                    <i className="size-1 animate-bounce rounded-full bg-current [animation-delay:-0.2s]" />
                    <i className="size-1 animate-bounce rounded-full bg-current [animation-delay:-0.1s]" />
          ...
          <div className="shrink-0 border-t border-border/70 p-4 sm:p-5">
            {attachments.length ? (
              <div className="mb-3 flex max-h-24 flex-wrap gap-2 overflow-y-auto pr-1">
                {attachments.map((attachment) => (
                  <div
                    key={attachment.id}
                    className="flex max-w-full items-center gap-2 rounded-[10px] border border-border bg-muted/45 px-2.5 py-1.5 text-xs"
                  >
                    <FileIcon
                      aria-hidden="true"
            ...
            <div className="relative" ref={emojiPickerRef}>
              {isEmojiPickerOpen ? (
                <div
                  ...
                  className="absolute bottom-[calc(100%+0.75rem)] left-0 z-20 w-[min(21rem,calc(100vw-3rem))] overflow-hidden rounded-[15px] border border-border bg-background p-2 shadow-[0_18px_48px_rgba(29,30,34,0.16)]"
                >
                  <div className="no-scrollbar flex gap-1 overflow-x-auto border-b border-border/70 pb-2">
                    {emojiGroups.map((group, index) => (
                      <button
                        key={group.label}
                        type="button"
                        aria-label={group.label}
                        aria-pressed={index === activeEmojiGroup}
                        onClick={() => setActiveEmojiGroup(index)}
                        className={cn(
                          "flex size-8 shrink-0 items-center justify-center rounded-[10px] text-base transition-colors",
                          index === activeEmojiGroup
                            ? "bg-primary text-primary-foreground"
                            : "hover:bg-muted"
                        )}
                      >
                  ...
                  <p className="px-1 pt-2 text-xs font-medium text-muted-foreground">
                    {emojiGroups[activeEmojiGroup].label}
                  </p>
                  <div className="mt-1 grid grid-cols-8 gap-0.5">
                    {emojiGroups[activeEmojiGroup].emoji.map((emoji, index) => (
                      <button
                        key={`${emoji}-${index}`}
                        type="button"
                        aria-label={`Add ${emoji}`}
                        onClick={() => addEmoji(emoji)}
                        className="flex aspect-square items-center justify-center rounded-[10px] text-xl transition-[background-color,transform] hover:bg-muted active:scale-90"
                      >
                        {emoji}
                      </button>
                    ))}
              ...
              <div className="flex items-end gap-2 rounded-[15px] border border-border/80 bg-background p-2 transition-colors focus-within:border-foreground/45">
                ...
                <Button
                  ...
                  className="size-10 rounded-[10px] text-muted-foreground hover:text-foreground"
                >
                  <PaperclipIcon aria-hidden="true" />
                </Button>
                <Textarea
                  aria-label="Message your AI clone"
                  value={draft}
                  ...
                  className="max-h-28 min-h-10 resize-none border-0 bg-transparent px-1 py-2 text-sm shadow-none focus-visible:ring-0"
                  rows={1}
                />
                <Button
                  type="button"
                  ...
                  className={cn(
                    "size-10 rounded-[10px] text-muted-foreground hover:text-foreground",
                    isEmojiPickerOpen && "bg-muted text-foreground"
                  )}
                >
            ...
            <p className="clone-composer-hint mt-2 text-center text-[0.68rem] text-muted-foreground">
