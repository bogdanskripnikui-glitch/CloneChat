  ...
  monthly: [
    {
      ...
      features: ["1 voice profile", "5 generations per week", "Paste text and file upload"],
      cta: "Start free",
      featured: false,
    },
    {
      ...
      features: ["Unlimited generations", "Telegram chat imports", "Separate business and casual modes"],
    ...
    },
    {
      ...
      features: ["Shared style libraries", "Approval-ready draft pipeline", "Priority onboarding"],
      cta: "Request access",
      featured: false,
    },
  ],
...
export function DashboardShell() {
  ...
  useEffect(() => {
    function closeVoiceMenu(event: PointerEvent) {
      ...
      if (!(target instanceof Element) || !target.closest("[data-voice-menu]")) {
  ...
  function saveRename(voice: VoiceProfile) {
    ...
    setVoices((current) =>
      current.map((item) => (item.id === voice.id ? { ...item, name: nextName } : item))
    )
    cancelRename()
  }
  ...
  return (
    <main className="dashboard-surface relative h-svh overflow-hidden bg-background">
      <div className="dashboard-pattern pointer-events-none absolute inset-0" aria-hidden="true" />

      <div className="relative mx-auto h-full w-full max-w-[1720px] p-3 sm:p-4">
        <div className="grid h-full min-h-0 overflow-hidden xl:grid-cols-[5.5rem_minmax(0,1fr)]">
          ...
          <div className="flex min-w-0 flex-col">
            <header className="screen-reveal screen-reveal-delay-1 flex h-16 items-center gap-4 px-4 sm:px-5">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[0.96rem] font-semibold tracking-[-0.03em]">VOXFORM</p>
            ...
            <div
              className={cn(
                "min-h-0 flex-1 gap-4 px-4 pb-4 pt-6 sm:px-5 sm:pb-5 sm:pt-7",
                "flex"
              )}
            >
              {activeSection === "dashboard" ? (
                <section className="screen-shift screen-shift-visible screen-shift-delay-2 flex min-h-0 w-full flex-1 flex-col gap-5 sm:gap-6">
                  {voices.length === 0 ? (
                    <div className="flex min-h-0 flex-1 items-center justify-center py-10">
                      <div className="flex max-w-sm flex-col items-center text-center">
                        ...
                        <p className="mt-6 text-[0.65rem] font-medium uppercase tracking-[0.16em] text-muted-foreground">
                          Voices
                        </p>
                        ...
                        <p className="mt-4 max-w-[21rem] text-sm leading-relaxed text-muted-foreground">
                          Add writing samples and create a voice profile for drafts that sound like you.
                        </p>
                        <Button type="button" size="lg" className="mt-7" onClick={openVoiceDialog}>
                  ...
                  <div className="flex min-w-0 gap-3">
                    {voiceCards.map((voice, index) => {
                      ...
                      return (
                        <article
                          key={voice.id}
                          className={cn(
                            "relative h-[4.5rem] w-[14rem] shrink-0 rounded-[14px] border p-4 text-left",
                            isActive
                        ...
                        >
                          <button
                            ...
                            className="absolute inset-0 z-0 rounded-[14px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                          />
                          {editingVoiceId === voice.id ? (
                            <div className="relative z-20 flex h-full w-full items-center gap-1.5">
                              <input
                                ...
                                onKeyDown={(event) => {
                                  if (event.key === "Enter") saveRename(voice)
                                  if (event.key === "Escape") cancelRename()
                                }}
                                className={cn(
                                  "min-w-0 flex-1 border-0 bg-transparent text-sm font-medium outline-none",
                                  isActive
                            ...
                            <>
                              <div className="pointer-events-none relative z-10 flex h-full items-center justify-between gap-3">
                                <p className="w-full truncate pr-10 text-sm font-medium">{voice.name}</p>
                              </div>
                              <div data-voice-menu className="absolute top-1/2 right-3 z-20 -translate-y-1/2">
                                <button
                                  type="button"
                                  aria-label={`More actions for ${voice.name}`}
                                  aria-expanded={openVoiceMenuId === voice.id}
                    ...
                    <button
                      type="button"
                      onClick={openVoiceDialog}
                      className="h-[4.5rem] w-[14rem] shrink-0 rounded-[14px] border border-dashed border-border/70 bg-white/48 p-3 text-left"
                    >
                  ...
                  <div className="grid min-h-0 flex-1 gap-3 overflow-hidden lg:grid-cols-[minmax(0,1.12fr)_minmax(0,0.88fr)]">
                    <div className="flex min-h-0 flex-col overflow-hidden rounded-[16px] border border-border/70 bg-white/88 p-5 shadow-[0_14px_36px_rgba(29,30,34,0.06)] sm:p-6">
                      ...
                      <FieldGroup className="mt-3 flex min-h-0 flex-1 gap-2">
                        <Field className="min-h-0 flex-1">
                          <FieldLabel htmlFor="dashboard-source" className="text-sm">
                            Source text
                          </FieldLabel>
                          <Textarea
                            id="dashboard-source"
                    ...
                    <div className="flex min-h-0 flex-col overflow-hidden rounded-[16px] bg-primary p-5 text-primary-foreground shadow-[0_20px_52px_rgba(29,30,34,0.18)] sm:p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-[0.65rem] font-medium uppercase tracking-[0.16em] text-primary-foreground/70">
                            Launch
                          </p>
                          <h2 className="mt-1 text-[1.4rem] leading-[1] font-medium tracking-[-0.04em] text-balance">
                            Ready result
                      ...
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {Object.entries(outputTemplates).map(([value, config]) => {
                          const Icon = config.icon
                          return (
                            <button
                              key={value}
                              type="button"
                              onClick={() => setOutputKind(value as OutputKind)}
                              className={cn(
                      ...
                      <div className="mt-3 flex min-h-0 flex-1 flex-col rounded-[16px] bg-white p-4 text-foreground sm:p-5">
                        ...
                        <pre className="mt-3 min-h-0 flex-1 overflow-auto whitespace-pre-wrap text-sm leading-relaxed font-sans text-muted-foreground">
                          {resultText}
                        </pre>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={() => document.getElementById("dashboard-source")?.focus()}
                        >
                          Refine my samples
                        </Button>
                        <Button type="button" variant="outline" size="sm" onClick={() => setActiveSection("billing")}>
                          Unlock more outputs
                        </Button>
                      </div>
                    </div>
                ...
                <section className="screen-shift screen-shift-visible screen-shift-delay-2 flex min-h-0 w-full flex-1 flex-col">
                  <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                    <div className="max-w-[43rem]">
                      ...
                      <p className="mt-4 max-w-[37rem] text-base leading-relaxed text-muted-foreground">
                        The free tier proves the voice fit. Paid plans unlock Telegram imports, more modes,
                        unlimited drafts, and collaborative review.
                      </p>
                    </div>

                    <Tabs value={billingCycle} onValueChange={(value) => setBillingCycle(value as BillingCycle)}>
                  <TabsList aria-label="Billing cycle" className="h-11 rounded-[20px] bg-white p-1 shadow-[0_8px_20px_rgba(29,30,34,0.06)]">
                        <TabsTrigger
                          value="monthly"
                          className="h-full min-w-24 rounded-[10px] px-5 text-sm font-medium text-foreground/48 hover:text-foreground/72 data-active:bg-primary data-active:text-primary-foreground data-active:hover:text-primary-foreground data-active:shadow-none"
                        >
                          Monthly
                        </TabsTrigger>
                        <TabsTrigger
                  ...
                  <div className="no-scrollbar mt-7 flex min-h-0 flex-1 snap-x snap-mandatory gap-4 overflow-x-auto pb-2 overscroll-x-contain lg:grid lg:grid-cols-3 lg:overflow-visible lg:pb-0">
                    {billingPlans[billingCycle].map((plan) => (
                      <article
                        key={plan.name}
                        className={cn(
                          "flex min-h-[30rem] shrink-0 snap-center basis-[min(25rem,calc(100vw-3rem))] flex-col rounded-[16px] p-7 lg:min-h-0 lg:min-w-0 lg:basis-auto lg:p-8",
                          plan.featured ? "bg-primary text-primary-foreground" : "bg-white/84 text-foreground"
                        )}
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <p className={cn("text-sm", plan.featured ? "text-primary-foreground/70" : "text-muted-foreground")}>{plan.name}</p>
                            <p className="mt-3 text-[2.8rem] leading-none font-medium tracking-[-0.04em]">{plan.price}</p>
                            <p className={cn("mt-2 text-sm", plan.featured ? "text-primary-foreground/70" : "text-muted-foreground")}>{plan.note}</p>
                          </div>
                          {plan.featured ? (
                        ...
                        <p className={cn("mt-6 text-sm leading-relaxed", plan.featured ? "text-primary-foreground/78" : "text-muted-foreground")}>{plan.description}</p>

                        <div className="mt-8 flex flex-1 flex-col gap-4">
                          {plan.features.map((feature) => (
                            <div key={feature} className="flex items-start gap-3">
                              <CheckIcon aria-hidden="true" className="mt-0.5 size-4 shrink-0" />
                              <p className={cn("text-sm leading-relaxed", plan.featured ? "text-primary-foreground/82" : "text-foreground/78")}>{feature}</p>
                            </div>
                          ))}
                        </div>
                        ...
                        <Button
                          ...
                          size="lg"
                          className="mt-10"
                          onClick={() => setActiveSection(plan.featured ? "settings" : "dashboard")}
                        >
                          {plan.cta}
                          <ArrowRightIcon data-icon="inline-end" aria-hidden="true" />
                        </Button>
                  ...
                  <section className="screen-shift screen-shift-visible screen-shift-delay-2 flex min-h-0 w-full flex-1 flex-col gap-4">
                    <div className="flex min-h-0 flex-1 flex-col rounded-[28px] border border-border/70 bg-white/88 p-4 shadow-[0_14px_36px_rgba(29,30,34,0.06)]">
                      ...
                      <FieldGroup className="mt-4 flex flex-1 gap-3">
                        <Field>
                          <FieldLabel htmlFor="dashboard-name" className="text-sm">
                            Workspace
                          </FieldLabel>
                          <Input id="dashboard-name" defaultValue="Voxform preview" />
                        </Field>
                        <Field>
                          <FieldLabel htmlFor="dashboard-email" className="text-sm">
                            Login layer later
                          </FieldLabel>
                          <Input id="dashboard-email" defaultValue="Google auth coming soon" disabled />
                          <FieldDescription className="text-xs leading-relaxed">
      ...
      >
        <div className="relative">
          <h2 id="add-voice-dialog-title" className="sr-only">Create a new voice profile</h2>
          {isVoiceDialogReady ? (
            <button
              type="button"
              ...
              className="absolute right-3 top-3 z-20 inline-flex size-9 items-center justify-center rounded-[10px] bg-background/92 text-muted-foreground shadow-[0_8px_18px_rgba(29,30,34,0.1)] [animation:dialog-close-enter_180ms_cubic-bezier(0.23,1,0.32,1)_forwards] transition-[transform,background-color,color] duration-160 ease-[cubic-bezier(0.23,1,0.32,1)] hover:bg-background hover:text-foreground active:scale-[0.97]"
            >
              <XIcon aria-hidden="true" className="size-4" />
            </button>
          ) : null}
